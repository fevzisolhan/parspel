...
import type {
  DB, Product, Sale, SaleItem, KasaEntry, Cari, StockMovement,
  RuleViolation,
} from '@/types';
...
function satisYap(
  ...
  if (iskonto) {
    ...
    discountAmount = iskonto.type === 'percent'
  ...
  // Fiili tahsilat
  const fiiliTahsilat = tahsilat !== undefined
    ? tahsilat
    : (odeme === 'cari' ? 0 : total);
  ...
  const sale: Sale = {
    ...
    unitPrice: total / Math.max(1, resolvedItems.reduce((s, i) => s + i.quantity, 0)),
...
function cariEkle(
  ...
  if (existing) {
    return { nextDB: prevDB, error: `"${form.name}" adında bir cari zaten mevcut.` };
  }
  ...
  if (similar) {
    return { nextDB, error: `Uyarı: "${similar.name}" adında benzer bir cari mevcut.` };
  }
...
function cariDuzenle(
  ...
  const nextDB: DB = {
    ...
    cari: prevDB.cari.map(c =>
      c.id === id
        ? { ...c, ...form, id: c.id, balance: c.balance, createdAt: c.createdAt, updatedAt: nowIso }
        : c
    ),
  };
  return { nextDB };
...
function siparisIsleTamamlandi(
  prevDB: DB,
  orderId: string
): { nextDB: DB } {
  const nowIso = now();
  ...
  const orders = prevDB.orders.map(o =>
    o.id === orderId
      ? { ...o, status: 'tamamlandi' as const, stockCompleted: true, updatedAt: nowIso }
      : o
  );
...
describe('🏪 Kapsamlı Senaryo Testleri', () => {
  ...
  describe('Grup 2: Cari Satış → Kısmi Tahsilat → İade', () => {
    ...
    it('kısmi tahsilatlı cari satış → iade sonrası cari.balance sıfır olmalı', () => {
      ...
      const { nextDB: afterSale, saleId, violations: saleViolations } = satisYap(
        prevDB,
  ...
  describe('Grup 7: Cari CRUD', () => {
    ...
    it('7b — silinen cari updatedAt güncellenmeli', () => {
      ...
      const cariId = db1.cari[0].id;
      const createdAt = db1.cari[0].updatedAt;
    ...
    it('7c — cari düzenlendiğinde bakiye değişmemeli, updatedAt güncellenmeli', () => {
      ...
      const { nextDB: db3 } = cariDuzenle(db2Modified, cariId, { phone: '555' });
    ...
    it('7c — cari düzenlendiğinde id ve createdAt değişmemeli', () => {
      ...
      const { nextDB: db2 } = cariDuzenle(db1, cariId, { name: 'Yeni İsim', phone: '0555' });
    ...
    it('7d — benzer isimde cari eklendiğinde uyarı üretilmeli (similarity >= 70)', () => {
      ...
      const { nextDB: db1, error: err1 } = cariEkle(db0, { name: 'Ahmet Yilmaz' });
      expect(err1).toBeUndefined();
      expect(db1.cari.filter(c => !c.deleted).length).toBe(1);

      // Benzer isimle ikinci cari ekle: 'Ahmet Yılmaz' (ı → i farkı)
      // normalizeTR sonrası her ikisi de 'ahmet yilmaz' → similarity = 100 >= 70
      const { nextDB: db2, error: err2 } = cariEkle(db1, { name: 'Ahmet Yılmaz' });
  ...
  describe('🔬 Görev 1: Bug Condition Exploration — Nakit Satış → İade', () => {
    it('nakit satış → iade sonrası net kasa sıfır olmalı (bug condition)', () => {
      ...
      // Satış yap
      const { nextDB: afterSale, saleId, violations: saleViolations } = satisYap(prevDB, [{ productId: 'soba-001', qty: 1 }], 'nakit');
      expect(saleViolations).toHaveLength(0);

      // Kasa geliri kontrol
      const kasaGelir = afterSale.kasa.filter(k => !k.deleted && k.type === 'gelir').reduce((s, k) => s + k.amount, 0);
