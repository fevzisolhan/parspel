...
function offlineReply(db: DB, query: string): string {
  ...
  if (q.includes('risk') || q.includes('kritik') || q.includes('öneri') || q.includes('ipucu')) {
    ...
    if (kasaToplam < 5000) riskler.push(`ğŸ’¸ Kasa düşük: ${formatMoney(kasaToplam)}`);
    if (out > 0) riskler.push(`ğŸ“¦ ${out} ürün stok bitti`);
    if (low > 0) riskler.push(`âš ï¸ ${low} üründe az stok`);
    if (alacak > 50000) riskler.push(`ğŸ’³ Yüksek alacak: ${formatMoney(alacak)}`);
    if (db.orders.filter(o=>o.status==='bekliyor').length > 3) riskler.push(`ğŸšš ${db.orders.filter(o=>o.status==='bekliyor').length} bekleyen sipariş`);
...
${overdueMusteri.length>0?`- âš ï¸ GECİKMİÅ ALACAKLAR (30+ gün): ${overdueMusteri.slice(0,5).map(c=>`${c.name} ${c.days}gün ${formatMoney(c.balance)}`).join(', ')}`:'- âœ… Gecikmiş alacak yok'}
...
${db.products.filter(p=>!p.deleted).map(p=>`- ${p.name} (stok:${p.stock}, fiyat:${p.price}TL)`).join('\n')}

### Mevcut müşteriler (cari için kullan):
${db.cari.filter(c=>!c.deleted&&c.type==='musteri').slice(0,20).map(c=>`- ${c.name} (bakiye:${c.balance}TL)`).join('\n')}`;
}

const QUICK_PROMPTS = [
  { label: 'ğŸ“Š Bu Ay Analiz', prompt: 'Bu ayın satış performansını detaylı analiz et. Geçen aya göre büyüme/düşüş var mı? Kâr marjı nasıl?' },
  { label: 'ğŸ“¦ Stok Durumu', prompt: 'Stoklarımın durumunu değerlendir. Hangi ürünleri acil sipariş etmeliyim? Stok değerim ne kadar?' },
  { label: 'ğŸ’° Kâr Analizi', prompt: 'En kârlı ürünlerim hangileri? Hangi kategoride kâr marjı düşük? İyileştirme önerileri ver.' },
  { label: 'ğŸ”® Satış Tahmini', prompt: 'Aylık trend verilerime göre önümüzdeki ay için satış tahmini yap. Hangi ürünlere odaklanmalıyım?' },
  { label: 'ğŸ‘¤ Alacak Takibi', prompt: 'Gecikmiş alacaklarım var mı? Hangi müşterilerden tahsilat yapmalıyım? Öncelik sırası ver.' },
  { label: 'ğŸ­ Tedarik Analizi', prompt: 'Tedarikçilerimle ilgili durum nedir? Bekleyen siparişler var mı? Maliyet optimizasyonu için ne yapabilirim?' },
  { label: 'ğŸ’¡ Kritik Öneriler', prompt: 'İşletmem için şu an en kritik 5 aksiyon nedir? Öncelik sırasıyla listele.' },
  { label: 'ğŸ“ˆ Büyüme Stratejisi', prompt: 'Verilerime göre satışları artırmak için hangi stratejileri izlemeliyim? Hangi ürün/kategori potansiyeli var?' },
  { label: 'âš–ï¸ Net Sermaye', prompt: 'Net sermayem ne durumda? Kasa, alacak ve borçlarımı değerlendirerek finansal sağlığımı analiz et.' },
  ...
  return (
    <div style={{ padding: '16px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, padding: '8px 12px', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8 }}>
        <span>â˜ï¸</span>
        <p style={{ color: '#10b981', fontSize: '0.82rem', margin: 0 }}>API anahtarları Firebase'de şifreli saklanır â€” tüm cihazlarda geçerlidir.</p>
      </div>
      {loading ? (
        <div style={{ color: '#64748b', fontSize: '0.85rem', textAlign: 'center', padding: '20px 0' }}>Firebase'den yükleniyor...</div>
      ) : (
        <>
          <label style={{ display: 'block', color: '#94a3b8', fontSize: '0.82rem', marginBottom: 4 }}>ğŸ¤– Claude API Key (Anthropic â€” birincil)</label>
          <input value={ck} onChange={e => setCk(e.target.value)} placeholder="sk-ant-..." style={{ ...inp, marginBottom: 14 }} type="password" />
          <label style={{ display: 'block', color: '#94a3b8', fontSize: '0.82rem', marginBottom: 4 }}>âœ¨ Gemini API Key (Google â€” yedek)</label>
          <input value={gk} onChange={e => setGk(e.target.value)} placeholder="AIza..." style={{ ...inp, marginBottom: 18 }} type="password" />
          {msg && <div style={{ marginBottom: 12, fontSize: '0.82rem', color: msg.startsWith('âœ…') ? '#10b981' : '#ef4444', fontWeight: 600 }}>{msg}</div>}
...
export default function AIAsistan({ db, save, embedded = false }: Props) {
  ...
  const send = useCallback(async (text?: string) => {
    ...
    if (elapsed < 3000 && lastReq > 0) {
      ...
      setMessages(prev => [...prev,
        ...
        { role: 'assistant', content: `â³ Çok hızlı istek gönderiyorsunuz. Lütfen ${wait} saniye bekleyin.`, source: 'offline' }
      ]);
      return;
    }
  ...
  const sourceLabel: Record<string, { label: string; color: string; bg: string }> = {
    claude: { label: 'ğŸ¤– Claude', color: '#818cf8', bg: 'rgba(99,102,241,0.12)' },
    gemini: { label: 'âœ¨ Gemini', color: '#a78bfa', bg: 'rgba(139,92,246,0.12)' },
    offline: { label: 'ğŸ”Œ Çevrimdışı', color: '#64748b', bg: 'rgba(100,116,139,0.1)' },
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: embedded ? '100%' : 'calc(100vh - 140px)' }}>
      ...
      {!embedded && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16, padding: '16px 20px', background: 'linear-gradient(135deg, rgba(99,102,241,0.1), rgba(139,92,246,0.06))', borderRadius: 16, border: '1px solid rgba(99,102,241,0.2)' }}>
          <div style={{ width: 48, height: 48, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0, boxShadow: '0 4px 20px rgba(99,102,241,0.4)' }}>ğŸ¤–</div>
          <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
            ...
            <button onClick={() => setShowSettings(s => !s)} title="API Ayarları" style={{ background: showSettings ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 8, color: '#818cf8', padding: '7px 12px', cursor: 'pointer', fontSize: '0.9rem' }}>âš™ï¸</button>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 14, padding: '16px 20px', marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <h3 style={{ color: '#f1f5f9', fontWeight: 700, fontSize: '0.95rem' }}>âš™ï¸ API Ayarları</h3>
            <button onClick={() => { invalidateKeyCache(); setShowSettings(false); }} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '1.1rem' }}>âœ•</button>
          </div>
      ...
      {messages.length === 0 && !showSettings && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '20px 0 16px' }}>
            <div style={{ fontSize: '3rem', marginBottom: 10, opacity: 0.5 }}>ğŸ¤–</div>
            <h3 style={{ color: '#475569', fontWeight: 700, marginBottom: 6, fontSize: '0.95rem' }}>İşletmenizle ilgili her şeyi sorabilirsiniz</h3>
            <p style={{ color: '#334155', fontSize: '0.8rem', maxWidth: 360, lineHeight: 1.6 }}>{hasKeys ? 'Gerçek verilerinizi analiz ederek yanıt verir.' : 'âš ï¸ âš™ï¸ Ayarlar\'dan API anahtarını girin. İnternetsiz de temel sorulara yanıt verir.'}</p>
          </div>
      ...
      {/* Onay Kartı â€” DB işlemi bekliyor */}
        {pendingActions && !loading && (
          <div style={{ background: 'linear-gradient(135deg,rgba(16,185,129,0.1),rgba(16,185,129,0.04))', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 14, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: '1.1rem' }}>âš¡</span>
              <span style={{ color: '#10b981', fontWeight: 700, fontSize: '0.88rem' }}>İşlem Onayı</span>
              <span style={{ color: '#475569', fontSize: '0.78rem', marginLeft: 'auto' }}>Kaydetmek istiyor musunuz?</span>
            </div>
            ...
            {actionResult && actionResult.msgIdx === pendingActions.msgIdx && (
              <div style={{ color: actionResult.success ? '#10b981' : '#ef4444', fontSize: '0.82rem', fontWeight: 600, textAlign: 'center' }}>
                {actionResult.success ? 'âœ… ' : 'âŒ '}{actionResult.msg}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                onClick={() => { setPendingActions(null); setActionResult(null); }}
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, color: '#64748b', padding: '9px 16px', cursor: 'pointer', fontSize: '0.85rem' }}
              >İptal</button>
            </div>
          </div>
        )}
      ...
      <div style={{ marginTop: 12, display: 'flex', gap: 8, alignItems: 'flex-end' }}>
        <div style={{ flex: 1, position: 'relative' }}>
          <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
            placeholder={listening ? 'ğŸ¤ Dinleniyor...' : 'Sorunuzu yazın veya ğŸ¤ mikrofona basın...'}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
        ...
        <button
          ...
          title={autoSpeak ? 'Sesli okuma açık â€” kapat' : 'Sesli okuma kapalı â€” aç'}
          style={{
